...
//!
//! [`Matrix::rational_eigenvalues`] returns **only** the eigenvalues that lie in
//! ℚ (found exactly by the rational-root theorem). Irrational and complex
//! eigenvalues are roots of the exact characteristic polynomial that require
//! real/complex root *isolation* — that machinery belongs with the
//! arbitrary-precision complex root-finding work in `rustmath-complex`
//! (`BigComplex`/ball arithmetic), not here. Callers needing the full spectrum
//! should take [`charpoly_berkowitz`]'s exact polynomial and hand it to a root
//! isolator; this module intentionally exposes the polynomial rather than
//! fabricating floating-point "eigenvalues" on an exact path. (The legacy `f64`
//! QR iteration in `eigenvalues.rs` remains available for numeric work.)
...
mod tests {
    ...
    #[test]
    fn berkowitz_4x4_matches_sympy() {
        // SymPy: charpoly([[2,0,1,-3],[1,1,4,0],[0,2,-1,5],[3,1,0,2]])
        //   = x^4 - 4x^3 + 4x^2 + 2x - 60  (coeffs low→high: -60, 2, 4, -4, 1).
        ...
        .unwrap();
        let cp = charpoly_berkowitz(&a).unwrap();
        assert_eq!(cp.coefficients(), &[z(-60), z(2), z(4), z(-4), z(1)]);
    }

    #[test]
    fn berkowitz_with_fractional_entries() {
        // SymPy: charpoly([[1/2,1/3],[1,1]]) = x^2 - 3/2·x + 1/6, and that
        // quadratic has no rational roots (discriminant 19/12 is not a square),
        // so rational_eigenvalues must honestly return the empty set.
        let half = Rational::new(1, 2).unwrap();
        let third = Rational::new(1, 3).unwrap();
        let a = Matrix::from_vec(2, 2, vec![half, third, q(1), q(1)]).unwrap();
        let cp = a.charpoly_exact().unwrap();
        assert_eq!(
            cp.coefficients(),
            &[
        ...
        assert!(a.rational_eigenvalues().unwrap().is_empty());
    }

    #[test]
    fn rational_eigenvalue_with_nontrivial_denominator() {
        // SymPy: [[2/3,1],[0,5]] has charpoly x^2 - 17/3·x + 10/3 and
        // eigenvalues {2/3, 5} — exercises q > 1 in the rational-root theorem.
        let a = Matrix::from_vec(
            2,
            2,
            vec![Rational::new(2, 3).unwrap(), q(1), q(0), q(5)],
        )
        .unwrap();
    ...
    #[test]
    fn rational_eigenvalues_honest_seam_for_irrational_spectrum() {
        // SymPy: [[1/2,0,0],[0,0,2],[0,1,0]] has charpoly
        // x^3 - 1/2·x^2 - 2x + 1 and eigenvalues {1/2, ±√2}. Only the rational
        // eigenvalue 1/2 is (and must be) reported; ±√2 need real/complex root
        // isolation, which lives with the complex root-finding work
        // (rustmath-complex) — see the module docs.
        let a = Matrix::from_vec(
            3,
            3,
            vec![
                Rational::new(1, 2).unwrap(), q(0), q(0),
                q(0), q(0), q(2),
                q(0), q(1), q(0),
            ],
        )
        ...
        let evals = a.rational_eigenvalues().unwrap();
        assert_eq!(evals, vec![(Rational::new(1, 2).unwrap(), 1)]);

        // The eigenspace for the reported eigenvalue is 1-dimensional.
        let es = a.eigenspace(&Rational::new(1, 2).unwrap()).unwrap();
        assert_eq!(es.len(), 1);
    }
}
