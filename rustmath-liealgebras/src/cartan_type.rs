...
///
/// Cartan types are specified by a letter (A, B, C, D, E, F, G) and a rank (positive integer).
...
impl Display for CartanType {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}_{}", self.letter, self.rank)
    }
}

/// Parse a Cartan type from a string like "A3", "E8", etc.
impl std::str::FromStr for CartanType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        CartanType::new(letter, rank)
            .ok_or_else(|| format!("Invalid Cartan type: {}{}", letter_char, rank))
    }
}
