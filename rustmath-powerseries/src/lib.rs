...
//!   `series_core_impls`), transcendental functions (`exp`, `log`, `sqrt`,
//!   `sin`/`cos`, `integral`, `laplace`) live in `transcendental`, composition
//!   (`compose`/`try_compose`) and reversion (`reversion`, compositional
//!   inversion à la Lagrange) live on [`PowerSeries`] itself, and
//!   [`LaurentSeries`] (integral valuation, a **field** over a field) and
//!   [`PuiseuxSeries`] (rational exponents) are new.  A [`Precision`] regime and
//!   a lightweight [`SeriesRing`] parent model free vs fixed precision; each
//!   element type offers `with_precision`-style constructors wiring that regime
//!   through (capped `Fixed` vs exact-polynomial `Free` semantics), and all
//!   arithmetic propagates the minimum precision of its operands.
