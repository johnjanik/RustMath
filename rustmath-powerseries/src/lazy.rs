...
impl<R: Ring + 'static> LazyPowerSeries<R> {
    ...
    /// Truncate to a [`PowerSeries`] under a [`Precision`] regime.  A lazy
    /// series is *exact* (unbounded — the capped vs exact distinction does
    /// not apply to the lazy representation itself, only to truncations of
    /// it), so both `Fixed(n)` and `Free(d)` simply yield that many terms.
    pub fn approximate_with(&self, prec: Precision) -> PowerSeries<R> {
        self.approximate(prec.default_terms())
    }
...
mod tests {
    ...
    #[test]
    fn approximate_with_precision_regime() {
        use crate::precision::Precision;
        let g = LazyPowerSeries::<Rational>::from_fn(|n| q(n as i64));
        let fixed = g.approximate_with(Precision::Fixed(3));
        assert_eq!(fixed.precision(), 3);
        assert_eq!(fixed.coeff(2), &q(2));
        let free = g.approximate_with(Precision::Free(6));
        assert_eq!(free.precision(), 6);
        assert_eq!(free.coeff(5), &q(5));
    }
}
