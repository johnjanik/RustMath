...
- [ ] 🟥 **[FINITEFIELDS]** T-111 Post-freeze: fix `PrimeField::zero()/one()` panic, `fp_factor` on inseparable input, `discriminant()` deg≥4, O(n!) `resultant`  👤 @unassigned  ⏳
- [ ] 🟥 **[GROUPS]** T-112 Post-freeze: native Schreier–Sims/BSGS → unblocks Stauduhar descent in GALOIS  👤 @unassigned  ⛔ T-111  ⏳
- [ ] 🟨 **[SYMBOLIC]** T-113 Risch integration (Hermite reduction + Rothstein–Trager) + `Expr → (num,den)` normalizer  👤 @unassigned  ⏳
- [ ] 🟨 **[CODING]** T-114 Generic `LinearCode<F: Field>` + Berlekamp–Massey  👤 @unassigned  ⛔ T-111  ⏳
- [ ] 🟨 **[GEOMETRY]** T-115 Exact rational polyhedra: double description / Fourier–Motzkin  👤 @unassigned  ⏳
...
|---|---|---|:--:|:--:|---|
| B-01 | `PrimeField::zero()/one()` **panic** (`finitefields/src/prime_field.rs:266`) | Blocks every generic `F: Field` algorithm — a generic `LinearCode<F>` cannot be written | 2026-07-09 | 🛑 | T-111. `Ring::zero()` is a no-arg associated fn so it cannot know *p*; needs a sentinel or a signature rethink |
