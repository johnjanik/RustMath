...
fn bench_diff_trig(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let sin_x = Expr::sin(Expr::Var(x.clone()));
    let cos_x = Expr::cos(Expr::Var(x.clone()));
...
fn bench_diff_nested(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let x_squared = Expr::Var(x.clone()).pow(Expr::from(2));
...
fn bench_diff_product_chain(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let x_cubed = Expr::Var(x.clone()).pow(Expr::from(3));
    let sin_x = Expr::sin(Expr::Var(x.clone()));
    let exp_x = Expr::exp(Expr::Var(x.clone()));
...
fn bench_diff_high_order(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let mut expr = Expr::Var(x.clone()).pow(Expr::from(20));

    let start = Instant::now();
    for _ in 0..iterations {
        let mut result = expr.clone();
        for _ in 0..10 {
            result = result.differentiate(&x);
        }
    }
    let elapsed = start.elapsed();
...
fn bench_expand_binomial(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let y = Symbol::new("y");
    let expr = (Expr::Var(x) + Expr::Var(y)).pow(Expr::from(10));
...
fn bench_simplify_rational(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let numerator = Expr::Var(x.clone()).pow(Expr::from(2)) - Expr::from(1);
    let denominator = Expr::Var(x.clone()) - Expr::from(1);
...
fn bench_substitution(iterations: usize) -> f64 {
    let x = Symbol::new("x");
    let expr = Expr::Var(x.clone()).pow(Expr::from(5))
             + Expr::from(3) * Expr::Var(x.clone()).pow(Expr::from(2))
             - Expr::from(7) * Expr::Var(x.clone())
             + Expr::from(2);

    let start = Instant::now();
    for _ in 0..iterations {
        let _result = expr.substitute(&x, &Expr::from(2));
    }
    let elapsed = start.elapsed();
