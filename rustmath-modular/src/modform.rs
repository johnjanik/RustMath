...
impl EisensteinSeries {
    ...
    pub fn coefficient(&self, n: u64) -> Rational {
        ...
        Rational::from_integer(Integer::from(sigma))
    }
...
impl JInvariant {
    ...
    pub fn coefficient(&self, n: i64) -> Integer {
        match n {
            -1 => Integer::one(),
            0 => Integer::from(744),
            1 => Integer::from(196884),
            2 => Integer::from(21493760),
            _ => Integer::zero(), // Placeholder
        }
    }
}
...
pub mod dimensions {
    ...
    pub fn modular_forms_gamma0(weight: Weight, level: u64) -> u64 {
        ...
        // Delegate to the dimension formulas in `dims`, which use the exact
        // level-1 formula and the Cohen-Oesterle formula for N > 1. The old
        // `((k-1)*index)/12 + 1` approximation was wrong (e.g. it gave
        // dim M_12(SL2Z) = 1 instead of the correct 2).
    ...
    pub fn cusp_forms_gamma0(weight: Weight, level: u64) -> u64 {
        ...
        // Delegate to the exact/Cohen-Oesterle cusp-form dimension in `dims`.
        // The old `M_k - 1` shortcut assumed a single Eisenstein series and
        // gave dim S_12(SL2Z) = 0 instead of the correct 1 (generated by Delta).
...
mod tests {
    ...
    fn test_eisenstein_series() {
        ...
        // sigma_3(1) = 1
        assert_eq!(e4.coefficient(1), Rational::from_integer(Integer::from(1)));

        // sigma_3(2) = 1 + 8 = 9
        assert_eq!(e4.coefficient(2), Rational::from_integer(Integer::from(9)));
    }
    ...
    fn test_j_invariant() {
        ...
        assert_eq!(j.coefficient(-1), Integer::one());
        assert_eq!(j.coefficient(0), Integer::from(744));
    }
