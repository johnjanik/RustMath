...
/// Prime factorization of a positive integer `m` (trial division) as `(p, exponent)`.
fn factor_int(mut m: i64) -> Vec<(i64, u32)> {
    let mut out = Vec::new();
    let mut d = 2i64;
    while d * d <= m {
        if m % d == 0 {
...
/// `|(O_K/m·O_K)^*|` = `∏_{𝔭∣m} N(𝔭)^{k_𝔭−1}(N(𝔭)−1)` with `k_𝔭 = e_𝔭·v_p(m)`,
/// `N(𝔭) = p^{f_𝔭}` — the unit group order of the residue ring.
pub fn residue_unit_group_order(f: &[Integer], m: i64) -> usize {
    let mut total = 1usize;
    for (p, a) in factor_int(m) {
        let (_ord, primes) = crate::ideals::prime_ideals(f, p);
        for (_, e, fdeg) in &primes {
            let k = (*e as u32) * a;
            let nrm = (p as usize).pow(*fdeg as u32);
            total *= nrm.pow(k - 1) * (nrm - 1);
        }
    }
    total
}

/// Order of the image of `O_K^*` in `(O_K/m·O_K)^*` (reduction mod `m·O_K` is just
/// coordinates mod `m` in the integral basis): the size of the subgroup generated by
/// the unit generators (small units + `−1`) under multiplication mod `(m, O_K)`.
pub fn unit_image_order_mod_m(f: &[Integer], m: i64) -> usize {
    let ord = maximal_order_data(f);
    let rts = roots(f);
    let (reals, cplx) = embeddings(&rts);
    let b = if f.len() - 1 <= 2 { 30 } else { 6 };
    let mi = Integer::from(m);
    let reduce = |v: &[Integer]| -> Vec<Integer> {
        v.iter()
            .map(|c| {
                let r = c.clone() % mi.clone();
                if r.signum() < 0 {
                    r + mi.clone()
                } else {
                    r
    ...
    let mut gens: Vec<Vec<Integer>> = small_units(&ord, &reals, &cplx, b)
        .into_iter()
        .map(|(a, _)| reduce(&a))
        .collect();
    let neg_one: Vec<Integer> = ord.one().iter().map(|c| -c.clone()).collect();
    gens.push(reduce(&neg_one));

    let one = reduce(&ord.one());
    let mut classes: Vec<Vec<Integer>> = vec![one];
    let mut frontier = vec![0usize];
    while let Some(idx) = frontier.pop() {
        let cur = classes[idx].clone();
        for g in &gens {
            let prod = reduce(&ord.mul(&cur, g));
            if !classes.iter().any(|c| c == &prod) {
                classes.push(prod);
...
/// The **ray class number** `h_𝔪` for the finite modulus `𝔪 = m·O_K`:
/// `h_𝔪 = h_K · |(O_K/m·O_K)^*| / [O_K^* : O_{K,𝔪,1}^*]`, the unit index being the
/// order of the image of the unit group in `(O_K/m·O_K)^*`. Generalizes the class
/// number (`m=1`); the archimedean (narrow) part is [`narrow_class_number`].
/// Validated vs `gp bnrclassno`.
pub fn ray_class_number(f: &[Integer], m: i64) -> Option<usize> {
    let cg = crate::classgroup::class_group(f)?;
    let h: usize = cg.iter().product::<usize>().max(1);
    if m <= 1 {
        return Some(h);
    }
    let ru = residue_unit_group_order(f, m);
    let ui = unit_image_order_mod_m(f, m);
    Some(h * ru / ui)
}
...
mod narrow_tests {
    ...
    #[test]
    fn ray_class_number_matches_gp() {
        // Cross-checked against PARI/GP bnrclassno (finite modulus m·O_K).
        assert_eq!(ray_class_number(&iz(&[5, 0, 1]), 3), Some(4)); // Q(√−5), m=3
        assert_eq!(ray_class_number(&iz(&[5, 0, 1]), 5), Some(20)); // m=5 (5 ramifies)
        assert_eq!(ray_class_number(&iz(&[5, 0, 1]), 2), Some(4)); // m=2 (2 ramifies)
        assert_eq!(ray_class_number(&iz(&[1, 0, 1]), 3), Some(2)); // Q(i), m=3 (w=4)
        assert_eq!(ray_class_number(&iz(&[1, 0, 1]), 5), Some(4)); // Q(i), m=5
        assert_eq!(ray_class_number(&iz(&[6, -1, 1]), 2), Some(3)); // Q(√−23), m=2
        assert_eq!(ray_class_number(&iz(&[-2, 0, 1]), 3), Some(1)); // Q(√2), m=3
        assert_eq!(ray_class_number(&iz(&[1, 0, 1]), 1), Some(1)); // m=1 ⇒ h
    }
