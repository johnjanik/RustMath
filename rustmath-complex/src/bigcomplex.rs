...
impl ComplexField for BigComplex {
    ...
    fn with_precision(&self, precision: u64) -> Self {
        BigComplex {
            re: RealField::with_precision(&self.re, precision),
            im: RealField::with_precision(&self.im, precision),
        }
    }
    // `pow` uses the trait's default principal-branch `exp(w·ln z)`, which is
    // exactly this backend's model (finite dyadics, no specials).
}
...
mod tests {
    ...
    #[test]
    fn test_with_precision_and_pow() {
        // with_precision: widening keeps the value; narrowing rounds.
        let p = 200;
        let third = bf_int(1, p) / bf_int(3, p);
        let z = BigComplex::new(third.clone(), third);
        let w = ComplexField::with_precision(&z, 80);
        assert_eq!(ComplexField::precision(&w), 80);
        assert!(approx(w.real().to_f64(), 1.0 / 3.0, 1e-15));
        let wide = ComplexField::with_precision(&z, 400);
        assert_eq!(ComplexField::precision(&wide), 400);
        // default pow: i^2 = -1 (principal branch exp(w ln z))
        let i = BigComplex::i_prec(120);
        let two = BigComplex::from_integer(&Integer::from(2), 120);
        let m1 = ComplexField::pow(&i, &two);
        assert!(approx(m1.real().to_f64(), -1.0, 1e-25));
        assert!(approx(m1.imag().to_f64(), 0.0, 1e-25));
        // 0^0 = 1
        let z0 = BigComplex::zero_prec(120);
        assert!(ComplexField::pow(&z0, &BigComplex::zero_prec(120)).is_one());
    }
