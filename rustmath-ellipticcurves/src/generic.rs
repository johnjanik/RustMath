...
//! # Provenance
//!
//! Moved verbatim (module paths in doctests updated) from
//! `rustmath-schemes/src/elliptic_curves/generic.rs` during the B3
//! elliptic-curve canonicalization: this crate is the semantic home for
//! elliptic curves, and this module is the canonical pure-`rustmath-core`
//! `EllipticCurve<F: Field>`. `rustmath_schemes::elliptic_curves::generic`
//! remains available as a re-export shim, so existing schemes users compile
//! unchanged. The unit tests exercising this module live in
//! `rustmath-schemes/src/elliptic_curves/tests.rs` and run through that shim.
//!
//! # Naming
//!
//! The types here are deliberately NOT re-exported at the crate root: the
//! crate root already exports the over-Q `curve::{EllipticCurve, Point}`
//! (num-bigint backed). Use the path-qualified
//! `rustmath_ellipticcurves::generic::{EllipticCurve, Point}` for the generic
//! field version. Merging the two (over-Q vs generic) is DEFERRED to the
//! Phase 2 num-*-to-core normalization; see `curve.rs`.
//!
//! # Weierstrass Form
//!
...
impl<F: Field> EllipticCurve<F> {
    ...
    ///
    /// ```
    /// use rustmath_ellipticcurves::generic::EllipticCurve;
    /// use rustmath_rationals::Rational;
    ///
...
// rustmath-schemes/src/elliptic_curves/tests.rs and exercise it through the
// schemes re-export shim (which also validates the shim itself).
